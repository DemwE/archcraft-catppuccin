...
apply_geany() {
	#copy color scheme
	cp $geany_colors ${PATH_GEANY}/colorschemes

	# modify geany config
	sed -i ${PATH_GEANY}/geany.conf \
		-e "s/color_scheme=.*/color_scheme=$geany_colors_scheme/g" \
		-e "s/editor_font=.*/editor_font=$geany_font/g"
}

# Appearance --------------------------------
apply_appearance() {
	# open terminal ask for sudo password
	alacritty -e echo "please enter your password for copying gtk theme" $$	sudo cp -R $PATH_OBTS/themes/$THEME/$gtk_theme /usr/share/themes
